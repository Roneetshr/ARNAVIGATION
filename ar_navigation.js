...
function render_elements() {
   $.ajax({
    ...
    success: function(response){
        ...
        let steps = response.routes[0].legs[0].steps[0]
        for(let i = 0; i < steps.length; i++){
            let image;
            let distance = steps[i].distance;
            let instruction = steps[i].maneuver.instruction;
            if(instruction.incudes("Turn right")){
                image = turn_right
            }else if(instruction.incudes("Turn left")){
                image = turn_left
            }
            if(i>0){
                $("#scene_container").append(
                    <a-entity gps-entity-place = "latitude:${steps[i].maneuver.location[1]}; longitiude:${steps[i].maneuver.location[0]">
                        <a-image 
                        name = "${instruction}"
